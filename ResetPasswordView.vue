...
import axios from 'axios' // Ensure you have your axios instance configured at '@/axios'
...
onMounted(() => {
  // Extract email and token from URL query parameters when the component mounts
  email.value = route.query.email as string || '';
  token.value = route.query.token as string || '';

  // Optional: If email/token are missing from the URL, you might want to redirect
  if (!email.value || !token.value) {
    errorMsg.value = 'Missing email or token in the URL. Please use the link from your reset request.';
  }
})
...
async function handleResetPassword() {
  ...
  try {
    ...
    errorMsg.value = err.response?.data?.msg || 'Failed to reset password. Please try again.';
  } finally {
    isLoading.value = false;
  }
}
    ...
    <form @submit.prevent="handleResetPassword" v-else>
      <div class="form-group">
        <p class="instruction-text">
          Please retrieve the **TEMP_RESET_TOKEN** from your backend console output
          (for development purposes). The email **{{ email }}** is automatically set.
        </p>
      </div>
      ...
      <div class="form-group">
        ...
        <input 
          ...
          type="text" 
          v-model="token" 
          placeholder="Paste TEMP_RESET_TOKEN here" 
          required
      ...
      <p class="info-message">
        ...
        backend console for the TEMP_RESET_TOKEN.
      </p>
