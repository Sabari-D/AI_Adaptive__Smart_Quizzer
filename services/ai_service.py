...
# # --- CRITICAL FIX: Configure Gemini SDK to use Render's Environment Variable ---
# try:
#     # This reads the GEMINI_API_KEY from the system environment securely injected by Render
#     genai.configure(api_key=os.environ.get('GEMINI_API_KEY'))
# except Exception as e:
#     # If the key is missing/invalid, this prints a warning but allows the service to start
#     print(f"WARNING: Gemini SDK configuration failed during startup: {e}")
# # -------------------------------------------------------------------------


# # --- NLTK DEPENDENCY CHECK (Kept for completeness) ---
# try:
...
#     def __init__(self):
#         # Initializes the GenerativeModel object. It now uses the key set globally above.
#         self.model = genai.GenerativeModel('gemini-2.5-flash') 
...
# --- CRITICAL FIX: Configure Gemini SDK to use the standard GOOGLE_API_KEY ---
try:
    # NOW READS the GOOGLE_API_KEY from the system environment securely
    # The SDK is looking for GOOGLE_API_KEY by default, which is why we must use it.
...
class AIService:
    ...
    def __init__(self):
        # Initializes the GenerativeModel object. It now uses the key set globally above via genai.configure().
        self.model = genai.GenerativeModel('gemini-2.5-flash') 
