...
from bson.objectid import ObjectId
import hashlib # CRITICAL: Imported for sha256 hashing
import random # Imported for random choice on unanswered content
...
class AdaptiveService:
    ...
    def get_unanswered_chunk(self, user_id: str, content_document: dict):
        """
        Finds chunks from the current content document that the user has never
        answered before, prioritizing new content.
        """
        if not content_document.get('chunks'):
            return None
            
        # 1. Get hashes of all chunks that have ever been attempted by this user
        attempted_hashes = self.history.distinct(
            "chunk_hash", 
            {"user_id": user_id, "content_id": content_document['_id']}
        )
        
        unanswered_chunks = []
        
        # 2. Iterate through all chunks in the content document
        for chunk_text in content_document['chunks']:
            chunk_hash = hashlib.sha256(chunk_text.encode()).hexdigest()
            
            # Check if the hash is NOT in the set of attempted hashes
            if chunk_hash not in attempted_hashes:
                unanswered_chunks.append(chunk_text)
                
        if unanswered_chunks:
            # Prioritize returning an entirely new chunk if available
            return random.choice(unanswered_chunks)
            
        return None # All chunks have been answered at least once